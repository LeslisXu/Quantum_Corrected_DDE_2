...
def init_voltage_2d(nx, ny, V_leftBC, V_rightBC):
    """
    初始化二维网格上的电压，包含边界点，使其在 x 方向上从 V_leftBC 到 V_rightBC 均匀分布。
    ...
    返回:
    - V: 一维 numpy 数组，长度为 (nx+1) * (ny+1)，包含边界点
    - V2d: 二维 numpy 数组，形状为 (ny+1, nx+1)
    """
    # 在 x 方向生成均匀分布的电压值，包含边界点
    x_vals = np.linspace(V_leftBC, V_rightBC, nx + 1)

    # 将 x 方向的电压分布复制到每一行，得到 2D 数组
    V2d = np.tile(x_vals, (ny + 1, 1))
...
for Va_cnt in range(0, num_V + 2):
    ...
    # Initialize potential - 修复这里，使用一致的初始化方法
    if Va_cnt == 0:
    ...
    print(f"Expected size: {(params.num_cell_x + 1) * (params.num_cell_y + 1)}")
    ...
    while error_np > params.tolerance:
        ...
        print(f'residual_n = {residual_n}\tresidual_p = {residual_p}\tresidual_V = {residual_V}\terror_np = {error_np}')        
